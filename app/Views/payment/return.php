...
<p style="text-align:center">※お待ちください※</p>
    ...
    <form id="send_params" method="post" action="/payment/complete">
        <input type="hidden" name="trading_id" value="<?= $trading_id ?? 0 ?>">
        <input type="hidden" name="type" value="<?= $type ?? 0 ?>">
        <input type="hidden" name="result_message" value="決済手続完了しました。">
    </form>

    <?php else: ?>

    <form id="send_params" method="post" action="/payment/stop">
