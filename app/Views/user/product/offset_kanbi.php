...
$Price =  (new \App\Models\Service\PriceInterface())->getObject($client_code);

$LimitDate = (new \App\Models\Service\LimitInterface())->getObject($client_code);

$product_data = $Product->getFromID(5);
...
.post h2 {
    border-bottom: none;
    background-image: url(/img/product/kanbi/illust.png);
...
section.content .wrap_bonus_info h3 {
    ...
    background-image:url(/img/product/kanbi/h3_bg_offset.png);
...
<article class="post" style="padding-top:0">
    ...
    <h2>
        <img src="/img/product/kanbi/title_detail_offset.png?d=20220421-1" alt="RGBカラー表紙パック（オフセット）" width="100%" style="max-width:480px">
    </h2>

    <div id="wrap_content_spec">

        <h3>
            <img src="/img/product/kanbi/h3_content_offset.png" width="100%" alt="RGBカラー表紙パック内容" class="pc_mid_only">
            <img src="/img/product/kanbi/h3_content_offset_sp.png" width="100%" alt="" class="sp_only">
        </h3>
...
<p>
    ...
    表紙：オフセット4色フルカラー ＋ アート紙（アイベストW）180kg ＋ PPまたはマットPP<br>
    本文：オフセット スミベタ（黒）またはカンビ特色12色から選択 ＋ 上質紙またはルンバ<br>
    製本：中綴じ、または無線綴じ
</p>    

<p>
    <small>※ルンバシリーズにつきましては、絵柄に黒ベタが多い場合白抜けが発生する場合がございます。
    出来る限り白抜けを少なくするよう最善を尽くしますが、用紙の特性上完全には抑えることができません。
    あらかじめご了承くださいませ。</small>
</p>

<p class="buttons">
    <a href="https://kanbi-comic.com/guide/download/" target="_blank">原稿形式・テンプレートはこちら</a><p>
...
-->
        ...
        <tr>
            <th>価格</th>
            <td>
                <p>セット内容の全てを含んだお得な特別価格</p>

<?php if($b_discount_10per): ?>
                <p class="campaign_discount">【期間限定】<b><?= $text_limit_discount ?>
                総額から</b><strong><?= $text_discount_per ?></strong></p>
<?php endif; ?>
        ...
        <h3>
            <img src="/img/product/kanbi/h3_spec_offset.png" width="100%" alt="オフセット印刷基本仕様" class="pc_mid_only">
            <img src="/img/product/kanbi/h3_spec_offset_sp.png" width="100%" alt="" class="sp_only">
        </h3>
...
<?php endif; // early_limit ?>
    ...
    <h3><img src="/img/product/kanbi/h3_icon_offset_2.png">
<!--
...
<ul class="attention">
    ...
    <h3><img src="/img/product/kanbi/h3_icon_offset_3.png">納品1カ所送料無料</h3>


        <div class="bonus_detail">

        <p>東名阪エリア3カ所納品無料となります<!--（快適印刷さんへの納品無料とは別途に、1カ所への送料が無料となります）-->。</p>

        <ul class="attention">
            <li>分納4カ所目から、1カ所毎に＋1,780円となります。</li>
            <li>入稿時にイベント会場、ご自宅、書店などご希望の納品場所をご指定ください。</li>
            <li>納品先は国内に限ります。北海道・沖縄などエリア外への納品は事前にご相談ください。</li>
        </ul>
    ...
    <h3><img src="/img/product/kanbi/h3_icon_offset_4.png">スタジオYOU主催イベントに<i>サークル参加1SPご招待</i></h3>
...
<!--
    ...
    <h4 id="price">RGBカラー表紙パック価格表（オフセット　A5）</h4>

<?php if($b_discount_10per): ?>
        <p class="campaign_discount">【期間限定】<b><?= $text_limit_discount ?>総額から</b><strong><?= $text_discount_per ?></strong></p>
<?php endif; ?>
    ...
    <table>
        ...
        <thead>
            ...
            <?php foreach($matrix_a5['index_h'] as $key_h): ?>
                <th><?= $key_h ?></th>
            <?php endforeach; ?>
            </tr>
        </thead>
        ...
        <tbody>
            <?php foreach($matrix_a5['index_v'] as $key_v): ?>
            <tr>
                <th><?= $key_v ?></th>
                <?php foreach($matrix_a5['index_h'] as $key_h): ?>
                    <td><?= number_format($matrix_a5['data'][$key_h][$key_v]) ?></td>
                <?php endforeach; ?>
            </tr>
            <?php endforeach; ?>
        </tbody>
    ...
    <h4 id="price_b5">RGBカラー表紙パック価格表（オフセット　B5）</h4>
...
<?php endif; ?>
    ...
    <p class="buttons">
        <a href="/order/form?id=<?= $id ?>">
            <img class="button_img" src="/img/product/kanbi/button_2_form_offset.png?d=20220421-1" alt="入稿フォーム" width="100%" style="max-width:460px"></a>
    </p>



    <h4>RGBカラー表紙パックご提供一覧</h4>

    <div class="wrap_after_notes">

        <h5>特別印刷価格ご提供：関西美術印刷様</h5>

        <div class="after_notes_text">


<p>当セットは同人誌印刷会社・関西美術印刷様とのコラボにより生まれました。</p>
...
<p>当セットは、関西美術印刷様が印刷してくださいますので、ご入稿、ご入金先も関西美術印刷様となり、発送・納品も同様となります。</p>
  
<p>入稿された原稿に「不備」などがありましたら、PICOさまからご連絡が入ります。予め、ご承諾いただけますようお願いいたします。</p>

<p>また、入稿時期、仕様変更、印刷後の手直し等、印刷料金に過不足が出た場合も、関西美術印刷様から連絡が入ります。万一、新たなお支払いが生じた場合、関西美術印刷様にお支払いください。</p>
-->
<p>発注入金後、ご登録頂いた内容に誤り（ページ数の数え間違い等）があり差額が発生した場合、関西美術印刷様に直接お支払いください。</p>
